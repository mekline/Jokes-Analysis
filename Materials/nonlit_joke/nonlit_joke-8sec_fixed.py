...
class Experiment:
    ...
    def files(self):
        ...
        if self.run=='1':
            ...
            divide = {}
            for x in ['joke','nonjoke']:
                divide[x]=[i for i in all_stims if i[2]==x]
        
            m = open('data/'+self.subj+'_materials.csv','wb')
            m_wr = csv.writer(m,quoting=csv.QUOTE_MINIMAL)
            for x in range(len(all_stims)/2):
                m_wr.writerow([x%3+1]+divide['joke'][x])
                m_wr.writerow([x%3+1]+divide['nonjoke'][x])
            m.close()
    ...
    def make_order(self):
        ...
        divide = {}
        for x in ['joke','nonjoke']:
            divide[x]=[i for i in all_stims if i[3]==x]
        self.order = []

        for o in optseq:
            print o, len(divide['joke']),len(divide['nonjoke'])
            if o[4]=='NULL':
